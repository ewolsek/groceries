...
function getSkus(items) {
  return items.map ((item)=> item.SKUs)
}
...
function getFruits(items) {
  return items.filter ((items) => items.getFruits)
}
...
function getItemsByCategory(items, category) {
  return items.filter ((Item) => category.getItemsByCategory)
}
...
function getCheapItems(items) {
  return items.filter ((item) => getCheapItems <= 2.50)
}
